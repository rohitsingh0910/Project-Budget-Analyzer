...
#
# @auth_router.get("/output.html", response_class=HTMLResponse)
# def get_output(request: Request):
#     user = request.session.get("user")
#     if not user:
#         return RedirectResponse(url="/auth/login", status_code=303)
#     return templates.TemplateResponse(
...
#     data = request.form
#
#     # one-hot encoding for architecture
#     arch_options = ["Client server", "Multi-tier", "Multi-tier with web interface", "Stand alone"]
#     arch_encoding = {f"Architecture_{opt}": 0 for opt in arch_options}
#     arch_encoding[f"Architecture_{data['architecture']}"] = 1
#
...
async def output(request: Request):
    ...
    # --------------------ml-model------------------------------
    best_model = joblib.load(r"/home/mirafra/Downloads/Project Budget Analyzer/random_forest_model.pkl")
